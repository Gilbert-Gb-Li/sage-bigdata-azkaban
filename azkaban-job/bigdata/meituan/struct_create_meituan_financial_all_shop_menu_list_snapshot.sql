CREATE EXTERNAL TABLE if not exists `bigdata.meituan_financial_all_shop_menu_list_snapshot`(
  ...
  `meta_name` string
) COMMENT 'all shop menu list'
PARTITIONED BY (dt string)
ROW FORMAT DELIMITED
  FIELDS TERMINATED BY '\t'
  LINES TERMINATED BY '\n'
  STORED AS ORC
  LOCATION '/data/meituan/snapshot/meituan_financial_all_shop_menu_list';
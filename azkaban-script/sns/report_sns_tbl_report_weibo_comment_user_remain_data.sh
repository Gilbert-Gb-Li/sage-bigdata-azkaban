...
echo "############### 微博评论活跃留存统计 start #####################"
date=$1

mysql_table="tbl_report_weibo_daily_data"
tmp_mysql_remain_table="tmp_tbl_report_weibo_daily_data_comment_remain_data"
...
FROM (
    ...
    WHERE a.dt='${next_remain_date}' AND CAST(a.comment_count AS bigint) > 0
) a
JOIN (
  SELECT a.user_id
  FROM sns.tbl_ex_weibo_user_action_snapshot a
  JOIN sns.weibo_monitor_user b
  ON a.user_id=b.weibo_id
  WHERE a.dt='${date}' AND CAST(a.comment_count AS bigint) > 0
) b
...
FROM (
    ...
    JOIN sns.weibo_monitor_user b
    ON a.user_id=b.weibo_id
    WHERE a.dt='${third_remain_date}' AND CAST(a.comment_count AS bigint) > 0
) a
JOIN (
  SELECT a.user_id
  FROM sns.tbl_ex_weibo_user_action_snapshot a
...
FROM (
    ...
    JOIN sns.weibo_monitor_user b
    ON a.user_id=b.weibo_id
    WHERE a.dt='${seven_remain_date}' AND CAST(a.comment_count AS bigint) > 0
) a
JOIN (
  SELECT a.user_id
  FROM sns.tbl_ex_weibo_user_action_snapshot a

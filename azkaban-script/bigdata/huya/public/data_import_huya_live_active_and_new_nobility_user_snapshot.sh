...
hive_sql="
insert into bigdata.huya_live_active_nobility_user_snapshot partition(dt='${date}')
select
...
hive_sql1="
insert into bigdata.huya_live_new_nobility_user_snapshot partition(dt='${date}')
select 
      a.audience_id
from
(
    select 
          audience_id 
    from bigdata.huya_live_active_nobility_user_snapshot
    where dt = '${date}'
) a
left join
(

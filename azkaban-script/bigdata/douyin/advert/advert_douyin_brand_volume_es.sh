...
# 计算时间是T+2
# 依赖advert_douyin_brand_volume
# 导出到ES 品牌品类分析->品牌的声量指数
...
do
echo "++++++++++++++++++++++++++++++++导出品牌品类分析中 品牌的声量指数 到ES 统计周期'${cycle}'++++++++++++++++++++++++++++++++++++++"
date_reduce=`date -d "-${cycle} day $dayBeforeYesterday" +%Y-%m-%d`
es_sql="${es_sql}
insert into bigdata.advert_douyin_brand_volume_es partition(dt='${dayBeforeYesterday}',cycle=${cycle})
    SELECT '${stat_date}' AS stat_month,
           ...
           ${cycle},
           'douyin' platform
      FROM bigdata.douyin_advert_brand_volume t

...
today=$1
dayBeforeYesterday=`date -d "-0 day $today" +%Y-%m-%d`

hive -e "use bigdata;
insert overwrite table bigdata.advert_douyin_valid_user_daily_snapshot partition(dt='${dayBeforeYesterday}')
...
insert overwrite table bigdata.advert_douyin_valid_vedio_daily_snapshot partition(dt='${dayBeforeYesterday}')
...
insert overwrite table bigdata.advert_douyin_valid_topic_daily_snapshot partition(dt='${dayBeforeYesterday}')
...
insert overwrite table bigdata.advert_douyin_valid_comment_daily_snapshot partition(dt='${dayBeforeYesterday}')
...
insert overwrite table bigdata.advert_douyin_valid_kol_daily_snapshot partition(dt='${dayBeforeYesterday}')
...
insert overwrite table bigdata.advert_douyin_valid_kol_vedio_daily_snapshot partition(dt='${dayBeforeYesterday}')
select distinct short_video_id
from (select distinct author_id,short_video_id
      from douyin_video_daily_snapshot
      where dt='${dayBeforeYesterday}') a
join (select kol_id
      from advert_douyin_valid_kol_daily_snapshot
      where dt='${dayBeforeYesterday}') b
on a.author_id=b.kol_id;
insert overwrite table bigdata.advert_douyin_kol_valid_vedio_daily_snapshot partition(dt='${dayBeforeYesterday}')
...
join (select short_video_id
      ...
      where dt='${dayBeforeYesterday}') d
on c.short_video_id=d.short_video_id;
insert overwrite table bigdata.advert_douyin_valid_kol_topic_daily_snapshot partition(dt='${dayBeforeYesterday}')
select distinct a.challenge_id
...
join (select kol_id
      ...
      where dt='${dayBeforeYesterday}') b
on a.author_id=b.kol_id;
insert overwrite table bigdata.advert_douyin_valid_topic_kol_daily_snapshot partition(dt='${dayBeforeYesterday}')
select distinct a.challenge_id
from (select distinct challenge_id,short_video_id
      from douyin_video_daily_snapshot
      where dt='${dayBeforeYesterday}') a
join (select short_video_id
...
insert overwrite table bigdata.advert_douyin_valid_kol_comment_daily_snapshot partition(dt='${dayBeforeYesterday}')
...
join (select short_video_id
      ...
      where dt='${dayBeforeYesterday}') b
on a.short_video_id=b.short_video_id;
insert overwrite table bigdata.advert_douyin_valid_comment_kol_daily_snapshot partition(dt='${dayBeforeYesterday}')
select distinct a.comment_id
from (select distinct short_video_id,comment_id
      from douyin_video_comment_daily_snapshot
      where dt='${dayBeforeYesterday}') a

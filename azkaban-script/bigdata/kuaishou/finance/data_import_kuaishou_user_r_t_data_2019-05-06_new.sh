...
apk_name="com.smile.gifmaker"
    ...
    from(
        ...
        left join (
            select b20.app_package_name,b20.user_id,
                b20.video_like_count as like_count,
                if(c20.video_like_count is not null and c20.video_like_count>0,b20.video_like_count-c20.video_like_count,b20.video_like_count) as new_like_count,
                b20.video_comment_count,
                if(c20.video_comment_count is not null and c20.video_comment_count>0,b20.video_comment_count-c20.video_comment_count,b20.video_comment_count) as new_video_comment_count
            from(
                select a20.app_package_name,a20.user_id,sum(a20.video_like_count) as video_like_count,sum(a20.video_comment_count) as video_comment_count
                from (
                    SELECT t22.app_package_name,t22.user_id,if(t22.video_like_count>0,t22.video_like_count,0) as video_like_count,if(t22.video_comment_count>0,t22.video_comment_count,0) as video_comment_count
                    FROM (
                    ...
                    on t22.app_package_name=t23.app_package_name and t22.user_id=t23.user_id
                ) as a20 group by a20.app_package_name,a20.user_id
            ) as b20
            left join (
                select a20.app_package_name,a20.user_id,sum(a20.video_like_count) as video_like_count,sum(a20.video_comment_count) as video_comment_count
                from (
            ...
            on b20.app_package_name=c20.app_package_name and b20.user_id=c20.user_id
        ) as e
        ...
        left join (
            ...
            from(
                ...
                from (
                    SELECT t22.app_package_name,t22.user_id,t22.video_id
                    FROM (
                        select * from bigdata.kuaishou_challenge_video_data_daily_snapshot
                        WHERE dt='${date}' AND app_package_name='${apk_name}'
                        AND user_id is not null AND user_id !=''
                    ) as t22
                    join(
        ...
        left join (
            select b.app_package_name,b.user_id,
                max(b.comment_count_new) as user_comment_count,
                if(max(b.comment_count_old)>0,max(b.comment_count_new)-max(b.comment_count_old),max(b.comment_count_new)) as new_user_comment_count
            from(
                select a.app_package_name,a.user_id,count(distinct a.comment_id) as comment_count_new,0 as comment_count_old
                from (
                    SELECT a1.app_package_name,a1.comment_user_id as user_id,a1.comment_id
                    FROM (
                        select * from bigdata.kuaishou_short_video_comment_data_daily_snapshot
                        WHERE dt='${date}' AND app_package_name='${apk_name}'
                    ...
                    on a1.app_package_name=a2.app_package_name and a1.comment_user_id=a2.user_id
                ) as a group by a.app_package_name,a.user_id
                UNION ALL
                select a.app_package_name,a.user_id,0 as comment_count_new,count(distinct a.comment_id) as comment_count_old
                from (
                    SELECT a1.app_package_name,a1.comment_user_id as user_id,a1.comment_id
                    FROM (
                        select * from bigdata.kuaishou_short_video_comment_data_daily_snapshot
            ...
            group by b.app_package_name,b.user_id
        ) as g
        ...
        left join (
            ...
            from(
                ...
                from (
                    SELECT t22.app_package_name,t22.user_id,t22.talk_id,if(max(t22.comment_count) >0,max(t22.comment_count)+count(distinct t22.comment_id),count(distinct t22.comment_id)) as comment_count
                    FROM(
                        select * from bigdata.kuaishou_talk_comment_data_daily_snapshot
                        WHERE dt='${date}' AND app_package_name='${apk_name}'
                        AND user_id is not null AND user_id !=''
                    ) as t22
                    join(
                    ...
                    on t22.app_package_name=t23.app_package_name and t22.user_id=t23.user_id
                    group by t22.app_package_name,t22.user_id,t22.talk_id
                ) as a group by a.app_package_name,a.user_id
            ) as b
            left join (
                select a.app_package_name,a.user_id,sum(comment_count) as comment_count
        ...
        left join (
            ...
            from(
                select t22.app_package_name,t22.user_id,count(distinct t22.commodity_name) as commodity_count
                from (
                    select * from bigdata.kuaishou_user_commodity_info_data_origin_orc
                    where dt='${date}' AND app_package_name='${apk_name}'
                        AND user_id is not null AND user_id !=''
                ...
                on t22.app_package_name=t23.app_package_name and t22.user_id=t23.user_id
                group by t22.app_package_name,t22.user_id
            ) as a

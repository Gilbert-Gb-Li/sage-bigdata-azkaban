...
hive_sql="
    insert into bigdata.kuaishou_location_video_daily_snapshot partition(dt='${date}')
    ...
    SELECT data_generate_time,app_package_name,video_create_time,video_id,video_caption,user_id,user_name,
        ...
        address_name,address_count,location_id
    FROM (
        SELECT *, row_number() OVER (PARTITION BY video_id,location_id ORDER BY data_generate_time DESC) AS order_num
        FROM (
            SELECT data_generate_time,app_package_name,video_create_time,video_id,video_caption,user_id,user_name,
                kwai_id,video_comment_count,video_like_count,video_play_count,bg_music_name,user_avatar_url,video_magic_face_id,
                video_magic_face_name,video_location_city,video_location_address,video_location_id,video_shopping,video_advert,
            ...
            SELECT data_generate_time,app_package_name,video_create_time,video_id,video_caption,user_id,user_name,
                ...
                bg_musician_id,bg_musician_name,bg_musician_kwai_id,bg_music_type,bg_music_id,bg_music_upload_time,video_share_info,
                address_name,address_count,location_id
            FROM bigdata.kuaishou_location_video_daily_snapshot
            WHERE dt = '${yesterday}'
        ) as p
    ) as t
    WHERE t.order_num = 1;
    ...
    ALTER TABLE bigdata.kuaishou_location_video_daily_snapshot DROP IF EXISTS PARTITION (dt='${date}');
    "

    echo '################删除HDFS上的数据####################'
    hdfs dfs -rm -r /data/kuaishou/snapshot/kuaishou_location_video_daily_snapshot/dt=${date}

#!/bin/bash
#***********************************************************************************
# **  文件名称: xiaohongshu_category_volume.sh
# **  创建日期: 2018年10月25日 
# **  编写人员: zsk
# 
# **  输入信息: 
...
#表名称
LOGNAME="xiaohongshu_category_volume"
...
time=$(date "+%Y%m%d")
#正常执行hql
SQL="insert overwrite table bigdata.xiaohongshu_category_volume partition
  (CYCLE, dt) 
select t5.pre_pl_id pre_category_id, -- 父品类id
       t5.pre_pl_name pre_category_name, -- 父品类名称
       t5.pl_id category_id, -- 品类id
       t5.pl_name category_name, -- 品类名称
       t5.pl_depth, -- 品类深度
       max(t5.mention_content_interact_avg) over(partition by t5.pl_depth) mention_content_interact_max, -- 内容平均互动量的最大值
       min(t5.mention_content_interact_avg) over(partition by t5.pl_depth) mention_content_interact_min, -- 内容平均互动量的最小值
       t5.mention_content_interact_avg, -- 当前品牌内容平均互动量
       1 + ((100 - 1) / ((max(t5.mention_content_interact_avg)
        over(partition by t5.pl_depth)) -
       (min(t5.mention_content_interact_avg)
        over(partition by t5.pl_depth)))) *
       (t5.mention_content_interact_avg -
       (min(t5.mention_content_interact_avg)
        over(partition by t5.pl_depth))) industrys_volume, -- 品类声量
       '${CYCLE}' cycle, -- 统计周期
       '${DAY_ID}' dt -- 数据生成日期    
  from (select t4.pl_id,
               t4.pl_name,
               t4.pre_pl_id,
               t4.pl_depth,
               t4.pre_pl_name,
          ...
          from (select t3.pl_id,
                 ...
                 inner join (select pl_id,
                             ...
                             group by pl_id,
                                      ...
                                      pre_pl_name,
                                      pl_depth) t3
                    on t1.category_id = t3.relat_pl_id
                 group by t3.pl_id,
                          t3.pl_name,
                          t3.pre_pl_id,
                          t3.pl_depth,

#!/bin/bash
#***********************************************************************************
# **  文件名称: advert_xiaohongshu_brand_content_es.sh
# **  创建日期: 2018年10月25日 
# **  编写人员: zsk
# 
# **  输入信息: 
...
#表名称
LOGNAME="advert_xiaohongshu_brand_content_es"
...
insert overwrite table bigdata.advert_xiaohongshu_brand_content_es partition (CYCLE, dt) 
select regexp_replace('${DAY_ID}', '-', '') stat_month, -- 月
       unix_timestamp( '${DAY_ID}', 'yyyy-MM-dd' ) * 1000, -- ES分片
       t1.kol_id,
       'xhs' platform, -- 平台显示什么内容
       t1.kol_id platform_kol_id, -- 平台统一kolId
       -- t2.top_pl_id category_id,
       -- t2.top_pl_name category,
       t1.brand_id,
       t1.brand,
       t1.content_date,
       count(distinct t3.content_id) content_num,
       sum(t3.new_interact_num) interact_num,
       sum(t3.new_interact_num) / count(distinct t3.content_id) interact_use_avg_count,
	   '${DAY_ID}',
	   '${CYCLE}',
       '${CYCLE}' cycle,
       '${DAY_ID}' dt
  from (select kol_id,
         ...
         where cast(regexp_replace(dt, '-', '') as bigint) <=
           ...
           and cast(regexp_replace(dt, '-', '') as bigint) >
               cast(regexp_replace('${PRE_DAY_ID}', '-', '') as bigint)
         group by category_id,
                  ...
                  content_date) t1
 inner join (
 -- select tt1.id, tt1.name, tt2.id top_pl_id, tt2.name top_pl_name from (
			   select * from bigdata.advert_category  where dt='${RECENT_DAY_ID1}'
			   -- ) tt1
              -- inner join (select * from bigdata.advert_category  where  dt='${RECENT_DAY_ID1}') tt2
              -- on tt1.pl_1 = tt2.id
				 ) t2

...
insert overwrite table bigdata.advert_category_keywords partition (dt)
select md5(t1.keyword) keyword_id,
       ...
       t2.pl_10,
	   '${DAY_ID}' dt
  from (select tt1.pid, tt1.cid, tt1.name, tt1.keyword
          from (select tmp1.pid, tmp1.cid, tmp1.name, keyword
                  from (select tt1.pid, tt1.cid, tt1.name, tt1.keywords
                          from (select pid,
                                       cid,
                                       name,
                                       keywords,
                         ...
                         where length(tt1.keywords) > 0
                           and tt1.rn = 1) tmp1 LATERAL VIEW explode(split(tmp1.keywords, ',')) key as keyword) tt1
         group by tt1.pid, tt1.cid, tt1.name, tt1.keyword) t1

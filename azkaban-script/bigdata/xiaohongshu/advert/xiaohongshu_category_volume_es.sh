#!/bin/bash
#***********************************************************************************
# **  文件名称: xiaohongshu_category_volume_es.sh
# **  创建日期: 2018年10月25日 
# **  编写人员: zsk
# 
# **  输入信息: 
...
#表名称
LOGNAME="xiaohongshu_category_volume_es"
...
insert overwrite table bigdata.xiaohongshu_category_volume_es partition
  (CYCLE, dt) 
select regexp_replace('${DAY_ID}', '-', '') stat_month, -- 月
       unix_timestamp('${DAY_ID}', 'yyyy-MM-dd') * 1000, -- ES分片
       category_id,
       category_deep,
       mention_content_interact_min,
       mention_content_interact_max,
       mention_content_interact_avg,
       industrys_volume, 
       dt,
	   cycle,
	   cycle,
	   dt
  from bigdata.xiaohongshu_category_volume
 where dt = '${DAY_ID}';"

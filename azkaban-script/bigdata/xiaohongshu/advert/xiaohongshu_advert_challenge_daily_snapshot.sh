#!/bin/bash
#***********************************************************************************
# **  文件名称: xiaohongshu_advert_challenge_daily_snapshot.sh
# **  创建日期: 2018年10月25日 
# **  编写人员: zsk
# 
# **  输入信息: 
...
#表名称
LOGNAME="xiaohongshu_advert_challenge_daily_snapshot"
...
time=$(date "+%Y%m%d")
#正常执行hql
SQL="insert overwrite table bigdata.xiaohongshu_advert_challenge_daily_snapshot partition (dt)
select temp2.record_time,
       temp2.challenge_look_count,
       temp2.challenge_name,
       temp2.challenge_desc,
       temp2.challenge_imag_url,
       temp2.challenge_author,
       temp2.play_count,
       temp2.challenge_id,
       '${DAY_ID}' dt
  from (select temp1.record_time,
               temp1.challenge_look_count,
               temp1.challenge_name,
               temp1.challenge_desc,
               temp1.challenge_imag_url,
               temp1.challenge_author,
               temp1.play_count,
               temp1.challenge_id,
               '${DAY_ID}' dt,
               row_number() OVER(PARTITION BY temp1.challenge_id ORDER BY temp1.record_time desc) rank
          from (select record_time,
                       ...
                       play_count,
                       challenge_id
                  from bigdata.advert_xiaohongshu_challenge_data_origin_orc
                 where dt = '${DAY_ID}' and length(challenge_id)>0
                union all
                select record_time,
                       challenge_look_count,
                       challenge_name,

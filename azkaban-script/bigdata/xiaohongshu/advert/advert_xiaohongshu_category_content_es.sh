#!/bin/bash
#***********************************************************************************
# **  文件名称: advert_xiaohongshu_category_content_es.sh
# **  创建日期: 2018年10月25日 
# **  编写人员: zsk
# 
# **  输入信息: 
...
#表名称
LOGNAME="advert_xiaohongshu_category_content_es"
...
insert overwrite table bigdata.advert_xiaohongshu_category_content_es partition
  (CYCLE, dt) 
select regexp_replace('${DAY_ID}', '-', '') stat_month, -- 月
  ...
  from (select kol_id,
         ...
         group by kol_id,
               ...
               content_id,
               content_date) t1
 inner join (select tt1.id, tt1.name, tt2.id top_pl_id, tt2.name top_pl_name
               from (select * from bigdata.advert_category  where dt='${RECENT_DAY_ID1}') tt1
              inner join (select * from bigdata.advert_category  where dt='${RECENT_DAY_ID1}') tt2
                 on tt1.pl_1 = tt2.id) t2

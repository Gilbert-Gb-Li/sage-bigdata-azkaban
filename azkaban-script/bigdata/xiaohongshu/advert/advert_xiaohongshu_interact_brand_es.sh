#!/bin/bash
#***********************************************************************************
# **  文件名称: advert_xiaohongshu_interact_brand_es.sh
# **  创建日期: 2018年03月06日 
# **  编写人员: zsk
# 
# **  输入信息: 
...
#表名称
LOGNAME="advert_xiaohongshu_interact_brand_es"
...
###############################以下为SQL编辑区########################################################
time=$(date "+%Y%m%d")
RECENT_DAY_ID11=$(hive -e "show partitions bigdata.advert_category;"|awk -F '/' '{print $1}'|awk -F '=' '{print $2}'|sort -u|head -n 1)
RECENT_DAY_ID22=$(hive -e "show partitions bigdata.advert_brand;"|awk -F '/' '{print $1}'|awk -F '=' '{print $2}'|sort -u|head -n 1)

#正常执行hql
SQL="add jar hdfs:/data/lib/es/commons-httpclient-3.1.jar;add jar hdfs:/data/lib/es/elasticsearch-hadoop-6.3.2.jar;
insert overwrite table bigdata.advert_xiaohongshu_interact_brand_es partition (dt)
select regexp_replace('${DAY_ID}', '-', '') stat_month, --月
       unix_timestamp('${DAY_ID}', 'yyyy-MM-dd') * 1000, --ES分片
       kol_id, --KOLID
       'xhs' platform, --平台
       kol_id platform_kol_id, --平台KOLID
       pl_1, --品类1
       pl_2, --品类2
       pl_3, --品类3
       pl_4, --品类4
       ...
       challenge_id, --话题id
       source_time, --数据产生日期
       content_brand_num, --内容提及品牌次数
       comment_brand_num, --评论提及品牌次数
	   '${DAY_ID}',
       '${DAY_ID}' dt --计算日期
  from (select kol_id,
               type,
               ...
               sum(keywords_num) over(partition by type, brand_id) content_brand_num,
               sum(keywords_num) over(partition by type, brand_id) comment_brand_num,
               brand_name brands,
               brand_id,
               category_id
          from bigdata.xiaohongshu_advert_source_keywords --分词表
  ...
  left join (select id,
             ...
             --品牌品类关键词对应关系表
              where dt = '${RECENT_DAY_ID11}') tmp3
    on tmp1.category_id = tmp3.id;
"
RunScript "${SQL}"

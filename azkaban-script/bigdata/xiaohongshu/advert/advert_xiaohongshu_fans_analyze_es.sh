...
#正常执行hql
SQL="
add jar hdfs:/data/lib/es/commons-httpclient-3.1.jar;add jar hdfs:/data/lib/es/elasticsearch-hadoop-6.3.2.jar;
WITH tmp1 AS (select t1.kol_id,
       top_cert,
       t1.like_count_rate, -- 点赞率
       t1.comment_fans_rate, --评论率
       t1.dive_rate, --潜水率
       t3.fans_user_id comment_user_id,
       case
         when age >= 0 and age <= 17 then
          '0-17'
         when age >= 18 and age <= 24 then
          '18-24'
         when age >= 25 and age <= 29 then
          '25-29'
         when age >= 30 and age <= 39 then
          '30-39'
         when age >= 40 and age <= 49 then
          '40-49'
         when age >= 50 and age <= 59 then
          '50-59'
         else
          '-1'
       end age_level, -- 年龄段
       ...
       prov, -- 省分
       comment_top_interest -- 兴趣
  from (select kol_id,
               sum(new_like_count) /
               sum(cover_fans_num + new_like_count) like_count_rate, -- 点赞率
               sum(comment_fans_num) /
               sum(cover_fans_num + new_like_count) comment_fans_rate, --评论率
               sum(cover_fans_num - comment_fans_num) /
               sum(cover_fans_num + new_like_count) dive_rate --潜水率
          from bigdata.xiaohongshu_advert_content_calc_data
         where dt = '${DAY_ID}'
           and cycle = '${CYCLE}'
         group by kol_id) t1
  left join (select tt1.kol_id, tt2.cert_1 top_cert
               from bigdata.advert_xiaohongshu_kol_mark_orc tt1
              inner join (select * from bigdata.advert_cert where dt='${CERT_DAY_ID}') tt2
                 on tt1.cert_id = tt2.id) t2
    on t1.kol_id = t2.kol_id
  left join (select tt3.user_id kol_id,
                    tt3.fans_user_id,
                    tt4.age,
                    tt4.sex,
                    tt4.prov,
              ...
              inner join (select user_id kol_id, age, sex, prov, city
               ...
               left join (select tt1.kol_id,
                 ...
                 on tt3.fans_user_id = tt5.kol_id) t3
    on t1.kol_id = t3.kol_id)
insert overwrite table bigdata.advert_xiaohongshu_fans_analyze_es partition
  (CYCLE, dt) 
select t4.stat_month,
  ...
  t4.col,
  t4.val,
  avg(t4.val) over(partition by t4.cert_id) avg_val,
       '${DAY_ID}' ,
	   '${CYCLE}' ,
	   '${CYCLE}' cycle,
       '${DAY_ID}' dt
...
kol_id,
'xhs' platform,
kol_id platform_kol_id,
top_cert cert_id,
'性别' type,
sex col,
count(1) val
from tmp1 where sex in ('男','女')
group by top_cert,kol_id,sex) t4
union all
select t4.stat_month,
  t4.timeindex,
  t4.kol_id,
...
age_level col,
count(1) val
from tmp1 where age_level<>'-1'
group by top_cert,kol_id,age_level) t4
union all
select t4.stat_month,
  t4.timeindex,
  t4.kol_id,
...
count(1) val
from tmp1 where length(comment_top_interest)>0
group by top_cert,kol_id,comment_top_interest) t4
union all
select t4.stat_month,
  t4.timeindex,
  t4.kol_id,
...
prov col,
count(1) val
from tmp1 where length(prov)>0
group by top_cert,kol_id,prov) t4
union all
select t4.stat_month,
  t4.timeindex,
  t4.kol_id,
...
'点赞率' col,
like_count_rate val
from  (select kol_id, top_cert, like_count_rate, comment_fans_rate, dive_rate
    from tmp1
   group by kol_id, top_cert, like_count_rate, comment_fans_rate, dive_rate) tmp2
union all
...
from  (select kol_id, top_cert, like_count_rate, comment_fans_rate, dive_rate
    from tmp1
   group by kol_id, top_cert, like_count_rate, comment_fans_rate, dive_rate) tmp2) t4;"

#!/bin/bash
#***********************************************************************************
# **  文件名称: xiaohongshu_brand_category_config.sh
# **  创建日期: 2018年10月25日 
# **  编写人员: zsk
# 
# **  输入信息: 
...
#表名称
LOGNAME="xiaohongshu_brand_category_config"
...
function CONFIGURE(){
	...
	SIXTY_BEFORE_DAY_ID=$(date -d "${DAY_ID}  -60 day" +%Y-%m-%d)
...
#正常执行hql  仅执行周期为60天的
#
SQL="insert overwrite table bigdata.xiaohongshu_brand_category_config
select t7.pre_pl_name, -- 上级品类                
       t7.pre_pl_id, -- 上级品类id              
       t7.pl_name, -- 品类                  
       t6.pl_id, -- 品类id                
       t8.pre_brand_name, -- 上级品牌                
       t8.pre_brand_id, -- 上级品牌id              
       t8.brand_name, -- 品牌                  
       t6.brand_id, -- 品牌id                
       t8.brand_depth, -- 品牌深度                
       t7.pl_depth, -- 品类深度                
       t6.relat_brand_id, -- 关联品牌id              
       t6.relat_pl_id -- 关联品类id              
  from (select t5.brand_id,
               t5.pl_id,
               t5.relat_brand_id,
               t5.relat_pl_id
          from (select brand_id,
                       ...
                       t4.relat_brand_id,
                       t4.relat_pl_id
                  from (select t1.brand_id    relat_brand_id,
                               t1.category_id relat_pl_id,
                               t2.path        category_path,
                               t3.path        brand_path
                          from (select brand_id,
                                       category_id,
                                       brand_name,
                                 ...
                                 where dt<='${DAY_ID}'
								 and dt>'${SIXTY_BEFORE_DAY_ID}'
                                 group by brand_id,
                                          category_id,
                                          brand_name,
                                          category_name) t1
                         inner join (select * from bigdata.advert_category  where dt='${RECENT_DAY_ID1}') t2
                            on t1.category_id = t2.id
                         inner join (select * from bigdata.advert_brand  where dt='${RECENT_DAY_ID2}') t3
                            on t1.brand_id = t3.id) t4 LATERAL VIEW explode(split(t4.brand_path, ',')) brand AS brand_id
                            LATERAL VIEW explode(split(t4.category_path, ',')) categroy AS pl_id) t5
         group by t5.brand_id,
                  t5.pl_id,
                  t5.relat_brand_id,
                  t5.relat_pl_id) t6
 inner join (select tmp1.id   pl_id,
                    tmp1.name pl_name,
					tmp1.depth pl_depth,
                    tmp1.pid   pre_pl_id,
                    tmp2.name pre_pl_name
               from (select * from bigdata.advert_category  where dt='${RECENT_DAY_ID1}') tmp1
               left join (select * from bigdata.advert_category  where dt='${RECENT_DAY_ID1}') tmp2
                 on tmp1.pid = tmp2.id) t7
    on t6.pl_id = t7.pl_id 
 inner join (select tmp1.id   brand_id,
               tmp1.name brand_name,
			   tmp1.depth brand_depth,
               tmp1.pid   pre_brand_id,
               tmp2.name pre_brand_name
          from (select * from bigdata.advert_brand  where dt='${RECENT_DAY_ID2}') tmp1
          left join (select * from bigdata.advert_brand  where dt='${RECENT_DAY_ID2}') tmp2
            on tmp1.pid = tmp2.id) t8 
            on t6.brand_id = t8.brand_id;
"

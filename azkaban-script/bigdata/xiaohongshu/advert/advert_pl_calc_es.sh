...
insert overwrite table bigdata.advert_pl_calc_es partition (cycle,dt)
select regexp_replace('${DAY_ID}', '-', '') stat_month, -- 月
       ...
       t3.prov, -- 省
       t3.city, -- 市
	   count(t2.content_id) over(partition by t1.pl_id,t2.kol_id) ,  -- KOL提及内容数
		 sum(t2.new_interact_num) over(partition by t1.pl_id,t2.kol_id),  -- kol提及互动量
       t1.pre_pl_id pre_industrys, --   父行业名称
	   t1.pl_id,
       t1.pl_name industrys, -- 行业
       count(t2.kol_id) over(partition by t1.pl_id) kol_count, -- 关联kol数
       sum(t2.cover_fans_num) over(partition by t1.pl_id) cover_fans_num, -- 覆盖粉丝
       sum(t2.interact_fans_num) over(partition by t1.pl_id) interact_fans_num, -- 互动粉丝
       (sum(t2.interact_fans_num) over(partition by t1.pl_id)) /
       (sum(t2.cover_fans_num) over(partition by t1.pl_id)) fans_interact_rate, -- 粉丝互动率
       count(t2.content_id) over(partition by t1.pl_id) content_num, -- 内容量
       sum(t2.new_interact_num) over(partition by t1.pl_id) content_interact_num, -- 互动量
       (sum(t2.new_interact_num) over(partition by t1.pl_id)) /
       (count(t2.content_id) over(partition by t1.pl_id)) content_interact_avg_num, -- 平均内容互动量
	   '${DAY_ID}',
	   	'${CYCLE}',
       '${CYCLE}' cycle, -- 统计周期
       '${DAY_ID}' dt -- 计算日期
  from (select t3.pl_id,
          ...
          from (select t2.pl_id,
                       ...
                       t2.pre_pl_name,
                       t1.content_id
                  from (select category_id, category_name, content_id
                         ...
                         where cast(regexp_replace(dt, '-', '') as bigint) <=
                           ...
                           and cast(regexp_replace(dt, '-', '') as bigint) >
                               cast(regexp_replace('${PRE_DAY_ID}', '-', '') as
                                    bigint)
                         group by category_id, category_name, content_id) t1
                 inner join (select pl_id,
                             ...
                             group by pl_id,
                                      ...
                                      pre_pl_id,
                                      relat_pl_id) t2
                    on t1.category_id = t2.relat_pl_id) t3
         group by t3.pl_id,
                  t3.pl_name,
                  t3.pre_pl_id,
                  t3.pre_pl_name,

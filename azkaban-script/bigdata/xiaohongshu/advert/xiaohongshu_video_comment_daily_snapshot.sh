#!/bin/bash
#***********************************************************************************
# **  文件名称: xiaohongshu_video_comment_daily_snapshot.sh
# **  创建日期: 2018年10月25日 
# **  编写人员: zsk
# 
# **  输入信息: 
...
#表名称
LOGNAME="xiaohongshu_video_comment_daily_snapshot"
...
time=$(date "+%Y%m%d")
#正常执行hql
SQL="insert overwrite table bigdata.xiaohongshu_video_comment_daily_snapshot partition (dt)
select temp2.record_time,
  ...
  from (select temp1.record_time,
               ...
               temp1.reply_id,
               temp1.comment_id,
               temp1.comment_nick_name,
               '${DAY_ID}' dt,
               row_number() OVER(PARTITION BY temp1.short_video_id,temp1.comment_id ORDER BY temp1.record_time desc) rank
          from (select record_time,
                       ...
                       comment_id,
                       comment_nick_name
                  from bigdata.advert_xiaohongshu_video_comment_data_origin_orc
                 where dt = '${DAY_ID}' and length(comment_id)>0
                union all
                select record_time,
                       created_time,
                       like_count,

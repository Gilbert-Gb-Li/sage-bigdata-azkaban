...
tmp_mysql_remain_table="tmp_tbl_report_comment_user_remain_data"
...
next_remain_sql="select '${next_remain_date}',count(distinct t1.user_id) as next_remain,1 from 
(select dt,user_id from ias.tbl_ex_short_video_comment_origin_orc where dt='${next_remain_date}' and user_id!='' and user_id is not null  and from_unixtime( cast( substr(created_time,0,10) as bigint),'yyyy-MM-dd')='${next_remain_date}' ) as t1 
join (
select dt,user_id from ias.tbl_ex_short_video_comment_origin_orc where dt='${date}' and user_id!='' and user_id is not null and from_unixtime( cast( substr(created_time,0,10) as bigint),'yyyy-MM-dd')='${date}'
) as t2 on t1.user_id=t2.user_id"

hiveSqlToMysql "${next_remain_sql}" "${next_remain_date}" "${tmp_mysql_remain_table}" "new_date,remain_count,remain_type" "new_date"


third_remain_sql="select '${third_remain_date}',count(distinct t1.user_id) as next_remain,3 from 
(select dt,user_id from ias.tbl_ex_short_video_comment_origin_orc where dt='${third_remain_date}' and user_id!='' and user_id is not null  and from_unixtime( cast( substr(created_time,0,10) as bigint),'yyyy-MM-dd')='${third_remain_date}') as t1 
join (
select dt,user_id from ias.tbl_ex_short_video_comment_origin_orc where dt='${date}' and user_id!='' and user_id is not null and from_unixtime( cast( substr(created_time,0,10) as bigint),'yyyy-MM-dd')='${date}'
) as t2 on t1.user_id=t2.user_id"
...
seven_remain_sql="select '${seven_remain_date}',count(distinct t1.user_id) as next_remain,7 from 
(select dt,user_id from ias.tbl_ex_short_video_comment_origin_orc where dt='${seven_remain_date}' and user_id!='' and user_id is not null  and from_unixtime( cast( substr(created_time,0,10) as bigint),'yyyy-MM-dd')='${seven_remain_date}') as t1 
join (
select dt,user_id from ias.tbl_ex_short_video_comment_origin_orc where dt='${date}' and user_id!='' and user_id is not null and from_unixtime( cast( substr(created_time,0,10) as bigint),'yyyy-MM-dd')='${date}'
) as t2 on t1.user_id=t2.user_id"
...
SET a.comment_user_next_remain=b.remain_count
...
ON a.stat_date=b.new_date
SET a.comment_user_third_remain=b.remain_count
...
ON a.stat_date=b.new_date
SET a.comment_user_seven_remain=b.remain_count

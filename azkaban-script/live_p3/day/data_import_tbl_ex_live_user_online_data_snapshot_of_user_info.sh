...
user_info_online_length="CREATE TEMPORARY TABLE default.tmp_p3_user_info_online_length AS
        ...
        from (
          select distinct appPackageName,user_id,dataSource,data_generate_time from ias_p3.tbl_ex_live_user_info_data_origin_orc where dt='${date}' and is_live=1 and user_id is not null and user_id!=''
        ) as p
        ) as a
        left join
        (
...
echo "############ 存储计算主播每次开播的时间#########"
save_to_hive_user_online_time="insert into live_p3.tbl_ex_live_user_online_time_daily_snapshot partition(dt='${date}')
...
user_online_snapshot="insert into live_p3.tbl_ex_live_user_online_daily_snapshot partition(dt='${date}')
...
delete_hive_partition="
   ALTER TABLE live_p3.tbl_ex_live_user_online_daily_snapshot DROP IF EXISTS PARTITION (dt='${date}');
   ALTER TABLE live_p3.tbl_ex_live_user_online_time_daily_snapshot DROP IF EXISTS PARTITION (dt='${date}');
  "

hdfs dfs -rmr /data/ias_p3/live/snapshot/tbl_ex_live_user_online_time_daily_snapshot/dt=${date}
hdfs dfs -rmr /data/ias_p3/live/snapshot/tbl_ex_live_user_online_daily_snapshot/dt=${date}

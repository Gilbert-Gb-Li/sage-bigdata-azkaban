...
echo "################# 删除快照、创建快照table、加载数据到live_p2.tmp_live_gift_val_file表   start ########################"

gift_val_table="tbl_ex_gift_val_snapshot"

drop_table_gift_val="drop table live_p2.${gift_val_table};"

create_table_gift_val="
CREATE TABLE IF NOT EXISTS live_p2.${gift_val_table}(
  id INT,
  app_id STRING,
  data_source STRING,
  gift_id STRING,
...
executeHiveCommand "${drop_table_gift_val} ${create_table_gift_val} LOAD DATA LOCAL INPATH '${tmp_live_gift_val_file}' INTO TABLE live_p2.${gift_val_table};"

rm -rf ${tmp_live_gift_val_file}

echo "################# 删除快照、创建快照table、加载数据到live_p2.tmp_live_gift_val_file表   end ########################"

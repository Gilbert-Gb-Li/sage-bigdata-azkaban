...
echo "############### 行业洞察-用户留存 报表统计 start #####################"

mysql_table="tbl_live_p2_insight_user_remain"
...
for app in ${live_app_list};
  do
    ...
    update_sql="
      ...
      JOIN (
        SELECT dt,biz_name,data_source,new_active_user_count
      ...
      SET a.new_count=b.new_active_user_count
    "
...
tmp_mysql_table="tmp_tbl_live_p2_insight_user_remain"
...
for remain_day in ${report_insight_remain_check_day};
  do
    ...
    hive_sql="
    SELECT '${new_day}', biz_name,data_source, COUNT(DISTINCT a.user_id)
    FROM
    (
      (SELECT user_id,biz_name,data_source FROM live_p2.tbl_ex_user_active_snapshot WHERE dt='${new_day}') a
      LEFT JOIN
      (SELECT user_id,biz_name,data_source FROM live_p2.tbl_ex_user_active_snapshot WHERE dt='${day}') b

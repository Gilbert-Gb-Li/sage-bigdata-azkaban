...
echo "############### 行业洞察-送礼物观众留存 报表统计 start #####################"

mysql_table="tbl_live_p2_insight_send_gift_audience_remain"
...
for app in ${live_app_list};
  do
    ...
    update_sql="
      ...
      JOIN (
        SELECT dt,biz_name,data_source,new_send_gift_audience_count
      ...
      SET a.new_count=b.new_send_gift_audience_count
    "
...
tmp_mysql_table="tmp_tbl_live_p2_insight_send_gift_audience_remain"
...
for remain_day in ${report_insight_remain_check_day};
  do
    ...
    FROM
    (
      (SELECT audience_id,biz_name,data_source FROM live_p2.tbl_ex_audience_send_gift_active_snapshot WHERE dt='${new_day}') a
      LEFT JOIN

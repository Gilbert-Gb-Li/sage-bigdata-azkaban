...
hive_sql="insert into live_p2.tbl_ex_live_user_known_daily_snapshot partition(dt='${date}')
select data_generate_time,app_package_name,data_source,search_id,live_id,user_id,is_live
    from(
        ...
        from (
            select data_generate_time,app_package_name,'${ias_source}' as data_source,search_id,live_id,user_id,is_live
            from ias_p2.tbl_ex_live_user_info_data_origin_orc
            where dt='${date}' and search_id is not null and user_id is not null and is_live=1
            union all
            select data_generate_time,app_package_name,data_source,search_id,live_id,user_id,is_live

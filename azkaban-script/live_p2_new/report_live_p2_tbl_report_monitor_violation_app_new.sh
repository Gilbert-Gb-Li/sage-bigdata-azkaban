...
echo "############### 直播监控 违规app  报表统计 start #####################"

mysql_table="tbl_live_p2_monitor_sex_app"
mysql_tmp_table="tmp_tbl_live_p2_monitor_violation_app"
...
echo "############### 直播监控 违规app 临时record表 start #####################"
...
echo "############### 直播监控 违规app  临时record表 end #####################"

echo "############### 直播监控 违规app  临时 start #####################"

hive_sql="
SELECT a.app_package_name as biz_name,
       a.start_time,
       a.end_time,
...
(
  ...
  (SELECT a.app_package_name,a.order_id,
    ...
    ${tmp_table} AS a
    LEFT JOIN
    (SELECT app_package_name,data_generate_time
  ...
  (SELECT a.app_package_name,a.order_id,
    ...
    ${tmp_table} AS a
    LEFT JOIN
    (SELECT app_package_name,data_generate_time,gift_val

...
mysql_app_table="tbl_live_p2_monitor_app_daily_data"
mysql_daily_table="tbl_live_p2_monitor_daily_data"
...
SELECT j.location,
       i.dt,
       i.app_package_name as biz_name,
       i.data_source,
       j.name,
       i.live_count,
       i.audience_count,
...
(
  SELECT '${day}' AS dt,
          x.app_package_name,
          x.data_source,
          x.active_user_count as live_count,
  ...
  FROM
  (
    SELECT * FROM live_p2.tbl_ex_live_platform_daily_snapshot WHERE dt='${day}' AND app_package_name in (${report_live_app_list})
  ) x
  LEFT JOIN
  (
    SELECT app_package_name,
        ...
        FROM live_p2.tbl_ex_live_platform_daily_snapshot AS a
        WHERE dt>='${last_seven_day}' AND dt<'${day}'
        GROUP BY app_package_name
  ) y
  ON x.app_package_name=y.app_package_name
) i
LEFT JOIN
(
  SELECT biz_name,location,max(name) AS name FROM live_p2.tbl_live_p2_app_location_all where location!='全国' and location_p in ('全国','美国')
  GROUP BY biz_name,location
) j
ON i.app_package_name=j.biz_name
"
